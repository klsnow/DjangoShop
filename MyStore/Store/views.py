...
#     response.set_cookie("login_form","login_page")#2 设置cookies值验证是否为login登陆进去的
...
def goods_list(request):
    ...
    if keywords:#搜索框的关键字
        goods_list = store.goods_set.filter(goods_name__contains=keywords)#从数据库中模糊查询

    else:
        goods_list = store.goods_set.all()#查询所有
